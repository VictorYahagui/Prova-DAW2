...
@section('content')
    <h2>Editar jogador: {{ $jogador->nome }}</h2>
    <div class="my-4">
    ...
    <div class="card">
        <div class="card-body">
        <form method="post" action="{{ route('jogadores.update', $jogador) }}">
                @csrf
                @method('PATCH')
                ...
                <div class="form-group mt-3">
                    ...
                    <input
                        ...
                        class="form-control"
                        value="{{ old('data_nascimento', $jogador->data_nascimento) }}"
                        required
                    />
                ...
                <div class="form-group mt-3">
                    ...
                    <select name="clube_id" class="form-control" required>
                        ...
                        @foreach($clubes as $clube)
                            <option
                                value="{{ $clube->id }}"
                                {{ intval(old('clube_id', $jogador->clube_id)) === $clube->id ? 'selected' : '' }}
                            >{{ $clube->nome }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group mt-3">
                    ...
                    <select name="posicao_id" class="form-control" required>
                        ...
                        @foreach($posicoes as $posicao)
                            <option
                                value="{{ $posicao->id }}"
                                {{ intval(old('posicao_id', $jogador->posicao_id)) === $posicao->id ? 'selected' : '' }}
                ...
                <div class="form-check mt-3">
                    <input
                        ...
                        value="true"
                        {{ boolval(old('possui_figurinha', $jogador->possui_figurinha)) ? 'checked' : '' }}
                    />
